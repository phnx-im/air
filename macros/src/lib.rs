...
struct FieldInfo {
    ...
    ty: syn::Type,
    tag: u32,
    /// `true` when the field type can be serialized as bytes with `serde_bytes`.
    is_bytes: bool,
    /// `true` when the field type is Option<T> where T can be serialized as bytes with
    /// `serde_bytes`.
...
/// Returns the single generic type argument of a single-segment path type (e.g. `Option<T>` ->
/// `T`).
...
/// Returns `true` when `ty` is syntactically `Option<T>` where `T` is can be serialized as bytes.
fn is_option_bytes_type(ty: &syn::Type) -> bool {
    if let Some(seg) = single_path_seg(ty)
        && seg.ident == "Option"
        && let Some(inner) = single_generic_arg(ty)
    {
        is_vec_u8(inner) || is_cow_u8_slice(inner)
    } else {
        false
    }
}

/// Returns `true` when `ty` is syntactically `Cow<'_, [u8]>`.
fn is_cow_u8_slice(ty: &syn::Type) -> bool {
    if let Some(seg) = single_path_seg(ty)
        && seg.ident == "Cow"
        && let syn::PathArguments::AngleBracketed(args) = &seg.arguments
        && args.args.len() == 2
        && matches!(args.args[0], syn::GenericArgument::Lifetime(_))
        && let syn::GenericArgument::Type(inner) = &args.args[1]
        && let syn::Type::Slice(slice) = inner
        && let Some(elem_seg) = single_path_seg(&slice.elem)
        && elem_seg.ident == "u8"
        && matches!(elem_seg.arguments, syn::PathArguments::None)
    {
        true
    } else {
        false
...
fn is_vec_u8(ty: &syn::Type) -> bool {
    if let Some(seg) = single_path_seg(ty)
        ...
        && inner_seg.ident == "u8"
        && let syn::PathArguments::None = inner_seg.arguments
    {
        true
    } else {
        false
...
fn skip_if_default_condition(fi: &FieldInfo) -> TokenStream2 {
    let ident = &fi.ident;
    let ty = &fi.ty;
    if is_vec(ty) || is_string(ty) || is_cow_u8_slice(ty) {
...
///
/// ```ignore
/// #[derive(SerializeTaggedMap, DeserializeTaggedMap)]
/// pub struct Foo {
///     #[tag(1)]
///     pub id: Uuid,
///     #[tag(2)]
...
/// }
/// ```
#[proc_macro_derive(SerializeTaggedMap, attributes(tag))]
pub fn derive_serialize_tagged_map(input: TokenStream) -> TokenStream {
    ...
    let fields = match &input.data {
        Data::Struct(s) => match &s.fields {
            Fields::Named(f) => f,
            _ => panic!("SerializeTaggedMap only supports named structs"),
        },
        _ => panic!("SerializeTaggedMap only supports structs"),
    };
    ...
    quote! {
        impl #impl_generics ::serde::Serialize for #name #ty_generics #where_clause {
                ...
                S: ::serde::Serializer,
            {
                ...
                let _len: usize = 0usize #(+ if #conditions { 1 } else { 0 })*;
                let mut _map = serializer.serialize_map(Some(_len))?;
                #(#entries)*
                _map.end()
            }
        }
...
pub fn derive_deserialize_tagged_map(input: TokenStream) -> TokenStream {
    ...
    let fields = match &input.data {
        Data::Struct(s) => match &s.fields {
            Fields::Named(f) => f,
            _ => panic!("DeserializeTaggedMap only supports named structs"),
        },
        _ => panic!("DeserializeTaggedMap only supports structs"),
    };
    ...
    // Adding 'de: 'a would make the impl too restrictive (it would no longer satisfy `for<'de>
    // Deserialize<'de>` bounds).
        ...
        .map(|fi| {
            ...
            if fi.is_bytes {
                if let Some(n) = &fi.array_len {
                    quote! {
                        #tag => {
                            #ident = _map.next_value::<::serde_bytes::ByteArray<#n>>()?
                                .into_array();
                        }
                    }
                } else if is_cow_u8_slice(&fi.ty) {
                    quote! {
                        #tag => {
                ...
                let inner_is_cow = single_generic_arg(&fi.ty).is_some_and(is_cow_u8_slice);
                if inner_is_cow {
                    quote! {
                        #tag => {
                            #ident = _map.next_value::<Option<::serde_bytes::ByteBuf>>()?
                                .map(|b| ::std::borrow::Cow::Owned(b.into_vec()));
                        }
                    }
                } else {
                    quote! {
