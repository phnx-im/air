...
pub(crate) mod tests {
    ...
    async fn load_before(pool: SqlitePool) -> anyhow::Result<()> {
        ...
        let (loaded, has_older) =
            ChatMessage::load_before(&mut txn, chat.id(), TimeStamp::from(35_000_000_000_i64), 2)
                .await?;
        ...
        let (loaded, has_older) =
            ChatMessage::load_before(&mut txn, chat.id(), TimeStamp::from(35_000_000_000_i64), 10)
                .await?;
    ...
    async fn load_after(pool: SqlitePool) -> anyhow::Result<()> {
        ...
        let (loaded, has_newer) =
            ChatMessage::load_after(&mut txn, chat.id(), TimeStamp::from(25_000_000_000_i64), 2)
                .await?;
        assert_eq!(loaded.len(), 2);
        ...
        let (loaded, has_newer) =
            ChatMessage::load_after(&mut txn, chat.id(), TimeStamp::from(25_000_000_000_i64), 10)
                .await?;
        assert_eq!(loaded.len(), 3);
        assert!(!has_newer);
    ...
    async fn load_around(pool: SqlitePool) -> anyhow::Result<()> {
        ...
        let (loaded, has_older, has_newer) =
            ChatMessage::load_around(&mut txn, chat.id(), TimeStamp::from(30_000_000_000_i64), 2)
                .await?;
        ...
        let (loaded, has_older, has_newer) =
            ChatMessage::load_around(&mut txn, chat.id(), TimeStamp::from(30_000_000_000_i64), 10)
                .await?;
