...
pub(super) struct JoinSigners<'a> {
    ...
    pub(super) self_group: Option<&'a ClientSigningKey>,
}

impl JoinSigners<'_> {
    /// The signing key matching the given joiner leaf signature key, if any.
    fn for_joiner_leaf(&self, signature_key: &SignaturePublicKey) -> Option<&ClientSigningKey> {
        [Some(self.client), self.self_group]
            .into_iter()
            .flatten()
            .find(|candidate| {
...
impl Group {
    ...
    pub(super) async fn join_apq_group(
        ...
        let signer = match t_key_material.key_package_bundle() {
            Some(bundle) => signers
    ...
    > {
        ...
        // Phase 3: Verify the user credentials. Relaxed for the self group:
...
async fn verify_member_credentials(
    ...
    // credential is not a valid AS-issued user credential are skipped with a
    ...
    for (credential, leaf_verifying_key) in unverified_credentials {
        match VerifiableUserCredential::verify_and_validate(
            credential,
            &leaf_verifying_key,
            None,
            &as_credentials,
