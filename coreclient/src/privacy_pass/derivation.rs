...
//!
//! ```text
//! batch_seed = Expand(PRK = token_seed,
//!     info = "air-pp-v1 batch" || u32_be(operation_type)
//!          || key_fingerprint || u32_be(allowance_epoch),
//!     L = 32)
//!
//! nonce_i = Expand(PRK = batch_seed,
//!     info = "air-pp-v1 nonce" || u32_be(i),
//!     L = 32)
//!
//! blind_i: for r = 0, 1, 2, ...:
//!     wide = Expand(PRK = batch_seed,
//!         info = "air-pp-v1 blind" || u32_be(i) || u32_be(r),
//!         L = 64)
...
const BATCH_LABEL: &[u8] = b"air-pp-v1 batch";
const NONCE_LABEL: &[u8] = b"air-pp-v1 nonce";
const BLIND_LABEL: &[u8] = b"air-pp-v1 blind";
...
pub(crate) fn batch_seed(
    ...
    let mut info = Vec::with_capacity(BATCH_LABEL.len() + 4 + key_fingerprint.len() + 4);
    info.extend_from_slice(BATCH_LABEL);
    info.extend_from_slice(&operation_type.to_be_bytes());
...
pub(crate) fn nonce(batch_seed: &[u8; SEED_LEN], index: u32) -> [u8; SEED_LEN] {
    let mut info = Vec::with_capacity(NONCE_LABEL.len() + 4);
    info.extend_from_slice(NONCE_LABEL);
...
fn expand<const L: usize>(prk: &[u8; SEED_LEN], info: &[u8]) -> [u8; L] {
