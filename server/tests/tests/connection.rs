...
/// The initiator asks for APQ, but the peer's leaf in the origin group does not announce
/// `apq_connection_groups`, so the connection group falls back to T.
#[tokio::test(flavor = "multi_thread", worker_threads = 1)]
#[tracing::instrument(name = "From-group connection falls back to T", skip_all)]
async fn add_contact_from_group_falls_back_to_t_without_peer_flag() {
    let mut setup = TestBackend::single().await;
    let alice = setup.add_user().await;
    let bob = setup.add_user().await;
    let charlie = setup.add_user().await;
    ...
    // Charlie runs a client that cannot join APQ connection groups.
    let charlie_user = &setup.get_user(&charlie).user;
    charlie_user
        .set_group_features(
            group_chat_id,
            AirFeatures {
                ...
                pq_groups: true,
                apq_connection_groups: false,
            },
        )
        .await
        .unwrap();

    // Bob learns about Charlie's leaf before connecting.
    let bob_user = &setup.get_user(&bob).user;
    let qs_messages = bob_user.qs_fetch_messages().await.unwrap();
    let result = bob_user.fully_process_qs_messages(qs_messages).await;
    assert!(
        result.errors.is_empty(),
        "Bob should process Charlie's leaf update without errors: {:?}",
        result.errors
    );
    ...
    let bob_user = &setup.get_user(&bob).user;
    let charlie_user = &setup.get_user(&charlie).user;
    assert_eq!(bob_user.chat_is_apq(bob_chat_id).await, Some(false));
    assert_eq!(charlie_user.chat_is_apq(charlie_chat_id).await, Some(false));
}
