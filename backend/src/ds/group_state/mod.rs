...
impl DsGroupState {
    ...
    pub(super) fn stage_welcome_info(&mut self, retained: RetainedWelcomeInfo) {
    ...
    ///
    /// Remove with the rest of the legacy welcome-info path.
    pub(super) fn legacy_welcome_info(
        self,
        epoch: GroupEpoch,
        // sender: &SignaturePublicKey,
    ) -> Option<DsWelcomeInfo> {
...
impl From<SerializableDsGroupStateV3> for DecodedDsGroupState {
    fn from(v3: SerializableDsGroupStateV3) -> Self {
        Self {
            ...
            // Kept for migration rather than dropped: a joiner whose Welcome
...
/// A decoded group state, plus whatever legacy welcome information the blob it
/// came from still carried.
...
/// Decode a room state snapshotted by V3. Returns `None` (and logs) on failure.
...
mod test {
    ...
    /// V3 kept the welcome information in the blob. Decoding one must hand it
    /// over rather than drop it, or a Welcome issued before the move to
    /// `ds_welcome_info` becomes unservable.
    #[test]
