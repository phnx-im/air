...
/// The three things `welcome_info` serves. Generic over the tree so a legacy
/// record, which is borrowed from the group state, fits too.
pub(super) type WelcomeInfoParts<Tree> = (
    Tree,
...
pub(super) struct DsWelcomeInfo {
    /// The ratchet tree as of this epoch. `Option` only because
    /// [`RatchetTree`] has no `Default`; a record without one is malformed.
    #[tag(1)]
    ...
    /// Unverified: the reader runs [`VerifiedRoomState::verify`]. `Option` for
    /// the same reason as `ratchet_tree`.
    #[tag(4)]
...
impl DsWelcomeInfo {
    ...
    /// creation time so it expires on the schedule it was written with.
    ...
    pub(super) fn into_parts(self) -> Option<WelcomeInfoParts<RatchetTree>> {
...
///
/// Request-scoped by construction: the update wrappers own one per group state
/// and hand it to the writer, so what is staged cannot be silently dropped and
/// nothing transient ends up on [`DsGroupState`](super::group_state::DsGroupState).
...
impl WelcomeInfoOutbox {
    ...
    pub(crate) fn take(self, epoch: GroupEpoch) -> Option<DsWelcomeInfo> {
        self.0
            .into_iter()
    ...
    /// Encrypt and store everything staged.
    ///
    /// Runs in the caller's transaction, alongside the group state, so an epoch
    /// is never reachable without the welcome information a joiner needs for it.
