...
impl StagedKeyPackages {
    ...
    /// Delete all staged key packages that are expired now.
    ///
    /// TTL of a staged key package batch is `STAGED_KEY_PACKAGES_BATCH_TTL`.
    pub(crate) async fn delete_expired_now(pool: &PgPool) {
        let cutoff = Utc::now() - STAGED_KEY_PACKAGES_BATCH_TTL;
        if let Err(error) = Self::delete_expired(pool, cutoff).await {
