  ...
  group('Primitive', () {
    ...
    test('holds no duplicate colors within a hue', () {
      for (final hue in Hue.values) {
        ...
        for (final shade in Shade.values) {
          final value = Primitive.chromatic(hue, shade).toARGB32();
          expect(
            seen[value],
            isNull,
            reason:
    ...
    test('neutral runs from pure white to pure black', () {
      expect(Primitive.neutral(NeutralShade.s0).toARGB32(), 0xFFFFFFFF);
      expect(Primitive.neutral(NeutralShade.s1000).toARGB32(), 0xFF000000);
    });

    test('every shade is fully opaque', () {
      for (final shade in NeutralShade.values) {
        expect(Primitive.neutral(shade).a, 1.0);
      }
      for (final hue in Hue.values) {
        for (final shade in Shade.values) {
          expect(Primitive.chromatic(hue, shade).a, 1.0);
        }
      }
    });
  });
